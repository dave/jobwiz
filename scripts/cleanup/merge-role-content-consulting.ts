...
/**
 * Merge role module content into Consulting company-role modules
 * Issue #300: Consulting batch
 *
 * This script:
 * 1. For each Consulting company-role module
...
function main() {
  console.log("Merging role content into Consulting company-role modules...\n");

  // Get all company-role modules for Consulting companies
  const files = fs.readdirSync(MODULES_DIR);
  const consultingFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return CONSULTING_COMPANIES.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${consultingFiles.length} Consulting company-role modules\n`);
  ...
  console.log(`Total: ${consultingFiles.length}`);
}
