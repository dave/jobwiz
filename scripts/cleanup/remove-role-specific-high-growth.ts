...
/**
 * Remove role-specific content from High-growth startup company modules
 * Issue #280: High-growth startups batch
 */
...
function fixInterviewerMindsetSection(section: Section, _companyName: string): boolean {
  ...
  for (const block of section.blocks) {
    ...
    if (block.type === 'tip' && block.content.text) {
      ...
      // Fix the behavioral questions tip
      if (text.includes('candidates commonly face: System design emphasis, Heavy focus on coding, Behavioral interviews important')) {
...
function fixTipsSection(section: Section, companyName: string): boolean {
  let changed = false;
  ...
  for (const block of section.blocks) {
    if (block.type === 'text' && block.content.text) {
      ...
      if (text.includes('**Insider Tips:**')) {
        // Replace role-specific tips with role-neutral ones
        const capitalizedName = companyName.charAt(0).toUpperCase() + companyName.slice(1);
        ...
        if (text.includes('LeetCode') || text.includes('coding') || text.includes('system design') ||
          ...
          block.content.text = `**Insider Tips:**\n\n- Research ${capitalizedName}'s recent news, product launches, and company direction\n- Prepare specific examples that demonstrate alignment with ${capitalizedName}'s values\n- Be ready to discuss how you've handled ambiguity and rapid change\n- Show genuine interest in ${capitalizedName}'s mission and the problems they're solving`;
          changed = true;
        }
      }
    }
...
function main() {
  console.log('Removing role-specific content from High-growth startup company modules...\n');

  let totalFiles = 0;
  let changedFiles = 0;
  const allChanges: { file: string; changes: string[] }[] = [];

  for (const company of HIGH_GROWTH_STARTUPS) {
