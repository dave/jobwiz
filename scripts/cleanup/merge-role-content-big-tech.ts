...
/**
 * Merge role module content into Big Tech company-role modules
 *
 * This script:
 * 1. For each Big Tech company-role module
...
function main() {
  console.log("Merging role content into Big Tech company-role modules...\n");

  // Get all company-role modules for Big Tech companies
  const files = fs.readdirSync(MODULES_DIR);
  const bigTechFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return BIG_TECH_COMPANIES.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${bigTechFiles.length} Big Tech company-role modules\n`);

  let processed = 0;
  let skipped = 0;
  let errors = 0;

  for (const file of bigTechFiles) {
    // Extract company and role from filename
    const match = file.match(/^company-role-([^-]+(?:-[^-]+)*)-(.+)\.json$/);
    if (!match) {
  ...
  console.log(`Total: ${bigTechFiles.length}`);
}
