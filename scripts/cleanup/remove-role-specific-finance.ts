...
/**
 * Remove role-specific content from Finance company modules
 * Issue #281: Finance batch
 */
    ...
    if (block.type === 'text' && block.content.text) {
      ...
      if (text.includes('**Insider Tips:**')) {
        ...
        if (text.includes('LeetCode') || text.includes('coding') || text.includes('system design') ||
          ...
          block.content.text = `**Insider Tips:**\n\n- Research ${companyName}'s recent news, deals, and market positions\n- Prepare specific examples that demonstrate alignment with ${companyName}'s values\n- Be ready to discuss how you've handled high-pressure situations\n- Show genuine interest in ${companyName}'s business and the financial services industry`;
          changed = true;
        }
      }
    }
...
function main() {
  console.log('Removing role-specific content from Finance company modules...\n');
