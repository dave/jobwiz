...
/**
 * Merge role module content into E-commerce/Retail company-role modules
 * Issue #301: E-commerce/Retail batch
 *
 * This script:
 * 1. For each E-commerce/Retail company-role module
...
function main() {
  console.log("Merging role content into E-commerce/Retail company-role modules...\n");

  // Get all company-role modules for E-commerce/Retail companies
  const files = fs.readdirSync(MODULES_DIR);
  const ecommerceFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return ECOMMERCE_COMPANIES.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${ecommerceFiles.length} E-commerce/Retail company-role modules\n`);
  ...
  for (const file of ecommerceFiles) {
    ...
    for (const company of ECOMMERCE_COMPANIES) {
  ...
  console.log(`Total: ${ecommerceFiles.length}`);
}
