...
/**
 * Remove role-specific content from Healthcare/Biotech company modules
 * Issue #284: Healthcare/Biotech batch
 */
...
function fixProcessSection(section: Section): boolean {
  ...
  for (const block of section.blocks) {
    if (block.type === 'text' && block.content.text) {
      ...
      // Fix Format line to be role-neutral - multiple patterns
      ...
      if (text.includes('**Format:** Mix of phone screen, technical, on-site')) {
        block.content.text = text.replace(
          '**Format:** Mix of phone screen, technical, on-site',
          '**Format:** Mix of phone screen, role-specific, on-site'
        );
        changed = true;
      }
      ...
      if (text.includes('**Format:** Mix of phone screen, technical, behavioral, and panel')) {
        block.content.text = text.replace(
          '**Format:** Mix of phone screen, technical, behavioral, and panel',
          '**Format:** Mix of phone screen, role-specific, behavioral, and panel'
        );
        changed = true;
      }
    }
    ...
    if (block.type === 'text' && block.content.text) {
      ...
      if (text.includes('**Insider Tips:**')) {
        ...
        if (text.includes('LeetCode') || text.includes('coding') || text.includes('system design') ||
          ...
          block.content.text = `**Insider Tips:**\n\n- Research ${companyName}'s recent initiatives and strategic priorities\n- Prepare specific examples that demonstrate alignment with ${companyName}'s values\n- Be ready to discuss how you've handled healthcare-related challenges or regulations\n- Show genuine interest in ${companyName}'s mission to improve patient outcomes`;
          changed = true;
        }
      }
    }
...
function main() {
  console.log('Removing role-specific content from Healthcare/Biotech company modules...\n');
