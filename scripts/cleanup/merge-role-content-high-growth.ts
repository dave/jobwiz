...
/**
 * Merge role module content into High-growth startups company-role modules
 * Issue #298: High-growth startups batch
 *
 * This script:
 * 1. For each High-growth startups company-role module
...
function main() {
  console.log("Merging role content into High-growth startups company-role modules...\n");

  // Get all company-role modules for High-growth startups companies
  const files = fs.readdirSync(MODULES_DIR);
  const startupFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return HIGH_GROWTH_STARTUPS.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${startupFiles.length} High-growth startups company-role modules\n`);
  ...
  console.log(`Total: ${startupFiles.length}`);
}
