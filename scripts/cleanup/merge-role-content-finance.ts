...
/**
 * Merge role module content into Finance company-role modules
 * Issue #299: Finance batch
 *
 * This script:
 * 1. For each Finance company-role module
...
function main() {
  console.log("Merging role content into Finance company-role modules...\n");

  // Get all company-role modules for Finance companies
  const files = fs.readdirSync(MODULES_DIR);
  const financeFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return FINANCE_COMPANIES.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${financeFiles.length} Finance company-role modules\n`);
  ...
  console.log(`Total: ${financeFiles.length}`);
}
