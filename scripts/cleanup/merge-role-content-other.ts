...
/**
 * Merge role module content into Other companies company-role modules
 * Issue #305: Other companies batch
 *
 * This script:
 * 1. For each Other companies company-role module
...
// Other companies (not fitting into previous categories)
...
function main() {
  console.log("Merging role content into Other companies company-role modules...\n");

  // Get all company-role modules for Other companies
  const files = fs.readdirSync(MODULES_DIR);
  const otherCompaniesFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return OTHER_COMPANIES.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${otherCompaniesFiles.length} Other companies company-role modules\n`);

  let processed = 0;
  let skipped = 0;
  let errors = 0;

  for (const file of otherCompaniesFiles) {
  ...
  console.log(`Total: ${otherCompaniesFiles.length}`);
}
