...
/**
 * Merge role module content into Healthcare/Biotech company-role modules
 * Issue #302: Healthcare/Biotech batch
 *
 * This script:
 * 1. For each Healthcare/Biotech company-role module
...
function main() {
  console.log("Merging role content into Healthcare/Biotech company-role modules...\n");

  // Get all company-role modules for Healthcare/Biotech companies
  const files = fs.readdirSync(MODULES_DIR);
  const healthcareFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return HEALTHCARE_COMPANIES.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${healthcareFiles.length} Healthcare/Biotech company-role modules\n`);
  ...
  console.log(`Total: ${healthcareFiles.length}`);
}
