...
/**
 * Merge role module content into Media/Entertainment company-role modules
 * Issue #304: Media/Entertainment batch
 *
 * This script:
 * 1. For each Media/Entertainment company-role module
...
const MEDIA_ENTERTAINMENT_COMPANIES = [
  "disney",
  "wbd",
  "spotify",
  "tiktok",
...
function main() {
  console.log("Merging role content into Media/Entertainment company-role modules...\n");

  // Get all company-role modules for Media/Entertainment companies
  const files = fs.readdirSync(MODULES_DIR);
  const mediaEntertainmentFiles = files.filter((f) => {
    if (!f.startsWith("company-role-")) return false;
    return MEDIA_ENTERTAINMENT_COMPANIES.some((company) =>
      f.startsWith(`company-role-${company}-`)
    );
  });

  console.log(`Found ${mediaEntertainmentFiles.length} Media/Entertainment company-role modules\n`);

  let processed = 0;
  let skipped = 0;
  let errors = 0;

  for (const file of mediaEntertainmentFiles) {
    // Extract company and role from filename
    let companySlug = "";
    let roleSlug = "";

    // Try each company to find the right match
    for (const company of MEDIA_ENTERTAINMENT_COMPANIES) {
  ...
  console.log(`Total: ${mediaEntertainmentFiles.length}`);
}
